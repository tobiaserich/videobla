...
def load_model():
    ...
    # LongCat-Video Model laden
    try:
        import sys
        sys.path.append("/app/LongCat-Video")
        ...
        # LongCat-Video Pipeline initialisieren
        MODEL = LongCatVideoPipeline.from_pretrained(
            model_path,
            torch_dtype=torch.float16 if DEVICE == "cuda" else torch.float32,
        ).to(DEVICE)
        
        # Compile für schnellere Inference (optional)
        if os.getenv("ENABLE_COMPILE", "false").lower() == "true":
            print("Compiling model for faster inference...")
            MODEL = torch.compile(MODEL)
        
        print(f"✅ LongCat-Video model loaded successfully!")
        print(f"   Device: {DEVICE}")
        print(f"   Dtype: {MODEL.dtype if hasattr(MODEL, 'dtype') else 'N/A'}")
        
    except ImportError as e:
        print(f"⚠️  LongCat-Video not installed, using dummy mode")
        print(f"   Error: {e}")
        # Fallback: Dummy model für Testing
        MODEL = {
            "device": DEVICE,
            "loaded": False,
            "dummy": True
